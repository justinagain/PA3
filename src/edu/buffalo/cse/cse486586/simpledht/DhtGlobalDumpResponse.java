...
public class DhtGlobalDumpResponse extends AsyncTask<DhtMessage, Void, Void>{

		private static final String TAG = DhtRequesGlobalDumpTask.class.getName();
		
		protected Void doInBackground(DhtMessage... msgs){
			try {
				Log.v(TAG, "About to push to socket a GlobalDumpResponse: " + Util.getPortNumber(msgs[0].getAvdOne()));
				...
				writeSocket.close();
				Log.v(TAG, "Pushed a GlobalDumpResponse!");
